...
from database.models.exercise import Exercise_structure
...
class ExerciseServicedb:
    ...
    def get_exercise(self, exercise_id: uuid.UUID) -> list:
        with session_factory() as session:
            try:
                query = select(Exercise_structure).filter_by(id=exercise_id).options(joinedload(Exercise_structure.field))
                res = session.execute(query)
                exercise = res.unique().scalars().all()
